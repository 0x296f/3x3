...
{
    scene_for_view->clear();// чистим сцену
    QGraphicsPixmapItem* Item = new QGraphicsPixmapItem(imgs); // говорим, что именно мы будем загружать в сцену
    scene_for_view->addItem(Item); // добавляем в сцену наше изображение


    ImageView* viewImage = new ImageView;
    viewImage->setScene(scene_for_view);

    Item->setTransformOriginPoint(Item->boundingRect().center());

    float scaleFactor = qMin(view->width() / Item->boundingRect().width(), viewImage->height() / Item->boundingRect().height());
